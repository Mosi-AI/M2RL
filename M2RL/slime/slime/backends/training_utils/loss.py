...
def compute_advantages_and_returns(args: Namespace, parallel_state: ParallelState, rollout_data: RolloutBatch) -> None:
    ...
    if args.advantage_estimator in ["grpo", "gspo"]:
        ...
        student_log_probs = [
            s_log_prob[-response_length:]
            for s_log_prob, response_length in zip(student_log_probs, response_lengths, strict=False)
        ]
        advantages = [
            teacher_log_prob[:min(len(teacher_log_prob),len(student_log_prob))] - \
                student_log_prob[:min(len(teacher_log_prob),len(student_log_prob))]
