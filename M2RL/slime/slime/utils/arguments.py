    ...
    def add_slime_arguments(parser):
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Number of worker processes for SFT rollout tokenization. "
                    "Defaults to min(cpu_count, 4) for memory safety. "
                    "Increase to 8-16 for more speed (requires more memory). "
                    "Set to 1 to disable parallel processing (serial mode)."
                ),
            )
        ...
        def add_reward_model_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=str,
                default=None,
                help="URL for the resources server for workplace_assistant environments, e.g. http://localhost:12000",
            )
            parser.add_argument(
                "--code-sandbox-url",
                type=str,
                default=None,
                help="URL for the code sandbox service for unit_test reward, e.g. http://localhost:8080/run_code",
            )
            parser.add_argument(
                "--custom-rm-path",
                type=str,
