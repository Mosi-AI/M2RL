...
ROLLOUT_ARGS=(
   --prompt-data $DATA_DIR/rl_train/multi_task.parquet
   --input-key prompt
   --label-key label
   --apply-chat-template
   --rollout-shuffle
   ...
   --code-sandbox-url http://localhost:8080/run_code
      --workplace-assistant-resources-server-url http://localhost:12000
...
EVAL_ARGS=(
   --eval-interval 20
   --eval-config $M2RL_ROOT/slime/eval_config/multi_task.yaml
)
